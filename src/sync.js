    ...
    return function(config, io) {
        ...
        sync.Array.prototype.at = function(index, update) {
            ...
            if (!!update) {
                ...
                io.call(config.svcName, 'update')(config.dbid, config.collection, data[index]._id, data[index], function(result) {
        ...
        sync.Array.prototype.pop = function() {
            var data = this.__data(),
                config = this.__config();
            io.call(config.svcName, 'remove')(config.dbid, config.collection, data[data.length - 1]._id, 
                function(result) {
                    if (result[0]) throw result[0];
                });
        ...
        sync.Array.prototype.push = function(obj) {
            ...
            io.call(config.svcName, 'add')(config.dbid, config.collection, obj, function(result) {
                if (result[0]) throw result[0];
            });
        ...
        sync.Array.prototype.shift = function() {
            ...
            io.call(config.svcName, 'remove')(config.dbid, config.collection, data[0]._id, function(result) {
                if (result[0]) throw result[0];
            });
            return data[0];
        };
        ...
        sync.Array.prototype.splice = function(index, num) {
            ...
            for (var i = num - 1; i >= 0; i--) {
                io.call(config.svcName, 'remove')(config.dbid, config.collection, data[index + i]._id, rmCb);
            }
